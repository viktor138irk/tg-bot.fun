...
const PLANS = {
  trial_week: { label:"Тест 7 дней", color:"#888",    maxBots:1,  rate:10, price:0    },
  ...
  postpaid_custom: { label:"Постоплата", color:"#b04ff7", maxBots:999, rate:5, price:null },
};
...
const PlanTag = ({plan})=>{const p=PLANS[plan]||PLANS.basic;return <Tag label={p.label} color={p.color}/>;};
  ...
  return(
    <Modal title="Пополнение баланса" onClose={onClose}>
      <div style={{background:`${R.accent}10`,border:`1px solid ${R.accent}25`,borderRadius:10,padding:"12px 14px",marginBottom:16}}>
        ...
        <div style={{fontSize:11,color:R.muted,marginTop:4}}>Комиссия {PLANS[tenant.plan]?.rate ?? PLANS.basic.rate}% списывается с каждого выполненного заказа</div>
      </div>
...
const TDashboard = ({tenant,shops,orders,onTopup,mobile})=>{
  ...
  return(<div>
    ...
    <div className="mob-grid2" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
      ...
      <Stat label="Комиссия" value={`${tenant.totalCommission.toLocaleString()}₽`} sub={`${PLANS[tenant.plan]?.rate ?? PLANS.basic.rate}%`} accent={R.purple} icon="percent" mobile={mobile}/>
    </div>
    <div className="mob-grid1" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      ...
      <Card><div style={{padding:"16px 18px"}}>
        ...
        {["Покупатель → переводит на вашу карту","Кассир подтверждает → бот выдаёт товар",tenant.plan === "postpaid_custom" ? `Начисляется ${PLANS[tenant.plan].rate}% в постоплату` : `Списывается ${PLANS[tenant.plan]?.rate ?? PLANS.basic.rate}% с баланса BotFactory`,`При ${tenant.alertThreshold}₽ → уведомление в Telegram`,"При 0₽ → боты приостанавливаются"].map((s,i)=>(
...
const TShops = ({shops,setShops,tenant,onTopup,toast,mobile})=>{
  ...
  const maxShops=(PLANS[tenant.plan]||PLANS.basic).maxBots;
...
const TOrders = ({orders,setOrders,tenant,onTopup,role,mobile})=>{
  ...
  const reject=id=>{setOrders(orders.map(o=>o.id===id?{...o,status:"rejected"}:o));setSel(null);};
...
const TBalance = ({tenant,setTenant,txs,onTopup,mobile})=>{
  ...
  return(<div>
    <SHead sup="Финансы" title="Мой баланс"
    ...
    <Card accent={c} glow={c} style={{marginBottom:20}}>
      <div style={{padding:mobile?"16px":"22px 28px"}}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
          <div>
            <div style={{fontSize:10,color:R.muted,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Баланс аккаунта (предоплата)</div>
...
export default function App() {
  ...
  const [tenant,setTenant] = useState(INIT_TENANT);
  ...
  const [platformTenants,setPlatformTenants] = useState(MOCK_PLATFORM_TENANTS);
  ...
  const handleTopup = amt => {
    if(amt<100) return;
    setTenant(t=>({...t,balance:t.balance+amt}));
    setTxs(t=>[...t,{id:Date.now(),type:"deposit",amount:+amt,date:new Date().toLocaleString("ru"),note:"Пополнение баланса"}]);
    showToast(`Баланс пополнен на ${amt.toLocaleString()} ₽`);
  };
  ...
  const SidebarContent = () => (
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflowY:"auto"}}>
      ...
      <div style={{padding:"10px 12px",borderBottom:`1px solid ${R.border}`,flexShrink:0}}>
        <div style={{fontSize:9,color:R.muted,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>Режим (демо)</div>
      ...
      <div style={{padding:"8px 12px",borderBottom:`1px solid ${R.border}`,background:`${modeColor}08`,flexShrink:0}}>
        ...
        {mode==="tenant"&&<div style={{fontSize:10,color:R.muted}}>Тариф: {(PLANS[tenant.plan]||PLANS.basic).label} · {(PLANS[tenant.plan]||PLANS.basic).rate}%</div>}
      </div>
      ...
      {mode==="tenant"&&(
        <div onClick={()=>{setTopupOpen(true);setSideOpen(false);}} style={{margin:"8px 10px",background:`${balColor}12`,border:`1px solid ${balColor}35`,borderRadius:9,padding:"9px 12px",cursor:"pointer",flexShrink:0}}>
          <div style={{fontSize:9,color:R.muted,textTransform:"uppercase",letterSpacing:"0.06em"}}>Баланс</div>
